...
const PublishedForm = () => {
    ...
    const exportToPDF = () => {
        ...
        const dynamicColumns = Object.keys(responses).map(id => ({
            id,
            title: responses[id].type || `Field ${id}`
        }));

        autoTable(doc, {
            head: [[...dynamicColumns.map(col => col.title)]],
            body: [[
                ...dynamicColumns.map(col =>
                    formatAnswerForPDF(responses[col.id]?.value ?? "")
                )
            ]],
            startY: 28,
            theme: "grid",
    ...
    const handleSubmitForm = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // wait for PDF save to complete before redirect
                setTimeout(() => {
                    Object.keys(localStorage).forEach((key) => {
                        if (key.startsWith("form_")) {
                            localStorage.removeItem(key);
    ...
    const handleBackPage = () => {
        ...
        if (pageId === "start") {
            ...
            if (currentIndex > 0) {
                ...
                navigate(`/forms/form-${formId}/page-start`);
            }
        }
    };
