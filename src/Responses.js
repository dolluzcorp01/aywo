...
const Responses = () => {
    ...
    const exportToExcel = () => {
        ...
        const wsData = [
            ...
            [`Generated by dForms | Form ID: ${cleanFormId}`],
            [],
            headers,
        ...
        const columnWidths = headers.map((header, i) => {
            const colValues = wsData.slice(4).map(row => String(row[i] || ''));
            const maxLength = Math.max(header.length, ...colValues.map(val => val.length));
